...
6. Panels whose geometry changed but whose derived fields did not are flagged as
   **suspect data**. A panel is excluded from the cost total **only when the
   suspect field feeds its own cost calculation**.

**Note on AC6.** An earlier draft excluded all suspect panels from the total.
That was wrong, and building the slice caught it. `remake` is a flat £180 and
never reads `weightKg`, so a stale weight makes the *record* untrustworthy, not
the *cost*. Excluding IW-L1-01 would report £540 against a true £720 — an
understatement at exactly the moment AC4 exists to prevent one. `recall`, whose
logistics may scale with weight, is the case where exclusion genuinely applies.
...
`firmware-event-schema-v4.2.md`. Requires `eventId`, `seq`, `bootId`,
