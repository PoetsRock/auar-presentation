...
*Caveat:* the sample emits one `nailing.progress` per panel, almost certainly an
artefact of a hand-made sample. If real firmware emits progress at 1Hz, nailing
runs ~120s per panel, so ~120 progress events replace 1 — roughly **18× the
total stream** (~2,500 events per run, still comfortably under 1 MB). Confirm
the real rate with Chris.

*Also note:* `EW-L1-E1` has no `frame.assembled` event, while the two panels
before it do. Either the event is optional or the sample is missing one, and
there is no way to tell which. **The sample demonstrates the exact defect this
